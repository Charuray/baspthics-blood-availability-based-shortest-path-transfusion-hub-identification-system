...
def submit_patient():
    ...
    try:
        ...
        insert_query = """
            INSERT INTO PATIENT (
                PID, NAME, DOB, GEN, BGRP, PHNO, EMAIL, ADDR, CITY,
                COND, ALLERGY, MEDS, VAX_STATUS, NOTES
            ) VALUES (
                :pid, :name, TO_DATE(:dob, 'YYYY-MM-DD'), :gen, :bgrp,
                :phno, :email, :addr, :city, :cond, :allergy, :meds, :vax_status, :notes
            )
        """
        ...
        conn.commit()
        flash("Patient details registered successfully!", "success")
...
def register_patient():
    ...
    return render_template('register_patient.html', pname=session.get('name'), user_id=session.get('user_id'))
